...
df1 = df1 %>%
  ...
  summarise(tot_cases = sum(cases)) %>%
  arrange(date) %>%
  mutate(cumulativecases = cumsum(tot_cases)) %>%
  filter(cumulativecases > 9) %>%
  mutate(
    days_elapsed = as.numeric(date - min(date)),
    end_date     = if_else(date == max(date), TRUE, FALSE),
    end_label    = if_else(end_date, country, NULL)
...
ggplot(data = df2,
  ...
  geom_line(size = 0.7, alpha = 0.8) +
  # add points to line endings
  geom_point(data = df2 %>% filter(end_date)) +
  # add country labels, nudged above the lines
  geom_label_repel(nudge_y = 1, direction = "y", hjust = 1) + 
  # turn off legend
  guides(color = FALSE) +
  ...
  # customize labels
  labs(
    x = "Days since 1st confirmed case",
...
```
## Tracking the number of active cases

The number of active cases is the number of confirmed cases minus recovered cases. 
